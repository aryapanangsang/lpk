...
$data= mysqli_query($con,"SELECT * FROM pelamar WHERE tujuan status='Proses'");
?>
...
<div class="card">
<div class="card-body">
    <h4 class="card-title" style="text-align: center;">Data Pelamar Diproses</h4>
    <div class="row">
        <div class="col-sm-3">    
...
<script>
        $(document).ready(function(){   
        $('#live-data').load('p_prosesGlobal.php');	             
        $('#proses').on('change', function(){        
        const selectedPackage = $('#proses').val();
        $('#selected').text(selectedPackage);
            if(selectedPackage == "Cikarang"){                
                $('#live-data').load('p_prosesCkr.php');	                         
            } else if(selectedPackage == "Purwakarta"){                
                $('#live-data').load('p_prosesPwk.php')
            }else{
                $('#live-data').load('p_prosesGlobal.php');	      
            }
        });
